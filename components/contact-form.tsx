    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
        ...
        <Reveal className="space-y-4 text-left">
          <form noValidate aria-busy={loading} onSubmit={handleSubmit} className="space-y-4">
...
function ContactLink({
  ...
  return (
    <a
      href={href}
      className={`flex items-center gap-4 ${accent ? "text-fuchsia-400 hover:text-fuchsia-300" : "text-white hover:text-fuchsia-400"} transition-colors`}
      target={href.startsWith("http") ? "_blank" : undefined}
      rel={href.startsWith("http") ? "noopener noreferrer" : undefined}
      aria-label={ariaLabel}
    >
      <div
        className={`w-10 h-10 p-2 rounded-full border ${accent ? "border-fuchsia-500/20" : "border-white/10"} flex items-center justify-center`}
      >
        {icon}
      </div>
    ...
    >
      <div className="max-w-sm rounded-lg border border-white/10 bg-black/70 backdrop-blur px-4 py-3 text-left">
        <div className="flex items-start gap-3">
          <div className={`mt-2 h-2 w-2 rounded-full ${success ? "bg-fuchsia-400" : "bg-red-400"}`} />
